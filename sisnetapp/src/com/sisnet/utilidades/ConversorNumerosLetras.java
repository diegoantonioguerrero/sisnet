...
public class ConversorNumerosLetras
{
  ...
  public void setCadenaNumero(String pCadenaNumero) {
    this.aCadenaNumero = mc.concatenarCadena(mc.obtenerSubCadena("000000000000", 0, 12 - mc.obtenerLongitudCadena(pCadenaNumero)), pCadenaNumero);
  }
  public int getModo() {
    return this.aModo;
  }
  ...
  private void llenarValores() {
    try {
      String[][] descripcionCifras_local = { { "UNA ", "DOS ", "TRES ", "CUATRO ", "CINCO ", "SEIS ", "SIETE ", "OCHO ", "NUEVE ", "UN " }, { "ONCE ", "DOCE ", "TRECE ", "CATORCE ", "QUINCE ", "DIECISEIS ", "DIECISIETE ", "DIECIOCHO ", "DIECINUEVE ", "" }, { "DIEZ ", "VEINTE ", "TREINTA ", "CUARENTA ", "CINCUENTA ", "SESENTA ", "SETENTA ", "OCHENTA ", "NOVENTA ", "VEINTI" }, { "CIEN ", "DOSCIENTAS ", "TRESCIENTAS ", "CUATROCIENTAS ", "QUINIENTAS ", "SEISCIENTAS ", "SETECIENTAS ", "OCHOCIENTAS ", "NOVECIENTAS ", "CIENTO " }, { "CIEN ", "DOSCIENTOS ", "TRESCIENTOS ", "CUATROCIENTOS ", "QUINIENTOS ", "SEISCIENTOS ", "SETECIENTOS ", "OCHOCIENTOS ", "NOVECIENTOS ", "CIENTO " }, { "MIL ", "MILLON ", "MILLONES ", "CERO ", "Y ", "UNO ", "DOS ", "CON ", "", "" } };
  ...
  private String obtenerEquivalenteUnidad() {
    ...
    try {
      ...
      if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) != '0' && mc.obtenerCarater(getCadenaNumero(), getPosicion() - 1) != '1' && (!mc.sonCadenasIguales(mc.obtenerSubCadena(getCadenaNumero(), auxiliar_local, auxiliar_local + 3), "001") || (getPosicion() != 2 && getPosicion() != 8)))
      {
        ...
        if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) == '1' && getPosicion() <= 6) {
          ...
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[0][Integer.parseInt(String.valueOf(mc.obtenerCarater(getCadenaNumero(), getPosicion()))) - 1]);
        } 
      }
      
      if ((getPosicion() == 2 || getPosicion() == 8) && !mc.sonCadenasIguales(mc.obtenerSubCadena(getCadenaNumero(), auxiliar_local, auxiliar_local + 3), "000"))
      {
    ...
    try {
      ...
      if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) != '0') {
        if (mc.obtenerCarater(getCadenaNumero(), getPosicion() + 1) == '0') {
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[2][Integer.parseInt(String.valueOf(mc.obtenerCarater(getCadenaNumero(), getPosicion()))) - 1]);
        }
        else if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) == '1') {
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[1][Integer.parseInt(String.valueOf(mc.obtenerCarater(getCadenaNumero(), getPosicion() + 1))) - 1]);
        }
          ...
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[2][Integer.parseInt(String.valueOf(mc.obtenerCarater(getCadenaNumero(), getPosicion()))) - 1] + getDescripcionCifras()[5][4]);
        } 
      }
  ...
  private String obtenerEquivalenteCentena() {
    ...
    try {
      ...
      if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) != '0') {
        ...
        if (mc.obtenerCarater(getCadenaNumero(), getPosicion()) == '1' && !mc.sonCadenasIguales(mc.obtenerSubCadena(getCadenaNumero(), auxliliar_local, auxliliar_local + 2), "00")) {
          
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[posicion1_local][9]);
        } else {
          numeroLetras_local = mc.concatenarCadena(numeroLetras_local, getDescripcionCifras()[posicion1_local][Integer.parseInt(String.valueOf(mc.obtenerCarater(getCadenaNumero(), getPosicion()))) - 1]);
        } 
      } 
  ...
  private void asignarValores(String pNumero, int pModo) {
    ...
    try {
      ...
      if (mc.verificarExistenciaSubCadena(cadenaNumero_local, String.valueOf('.'))) {
        ...
        cadenaDecimales_local = mc.obtenerSubCadena(cadenaNumero_local, posicionPunto_local + 1, posicionFinal_local);
  ...
  public String obtenerNumeroEnLetras(String pNumero, int pModo, String pValorInicial, String pValorEntero, String pValorSeparador, String pValorDecimal) {
      ...
      { obtenerValoresLetras();
        ...
        { for (int posicion_local = 0; posicion_local < 12; posicion_local++) {
            ...
            if (esDigitoSignificativo() || mc.obtenerCarater(getCadenaNumero(), posicion_local) != '0') {
              ...
              switch (posicion_local % 3) { case 0:
                  numeroLetras_local = obtenerEquivalenteCentena(); break;
                case 1: numeroLetras_local = obtenerEquivalenteDecena(); break;
                case 2: numeroLetras_local = obtenerEquivalenteUnidad(); break; }
          ...
          if (!existenDecimales_local) {
            ...
            if (mc.sonCadenasIguales(mc.obtenerSubCadena(getCadenaNumero(), longitudCadena_local - 6, longitudCadena_local), "000000"))
            {
          ...
          numeroLetras_local = null; }  }  if (mc.esCadenaNumerica(pNumero, false) && Double.parseDouble(pNumero) == 0.0D) setNumeroLetras(mc.concatenarCadena("CERO ", pValorEntero));  } catch (Exception excepcion) { excepcion.printStackTrace(); } finally { numeroLetras_local = null; }
    
    return getNumeroLetras();
  }
}
/* Location:              D:\Personal\sisnet\sisnetMasterApp\sisnetapp.war!\WEB-INF\classes\com\sisne\\utilidades\ConversorNumerosLetras.class
