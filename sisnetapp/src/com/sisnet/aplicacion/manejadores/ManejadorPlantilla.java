...
public class ManejadorPlantilla
{
  ...
  public void setRutaDescargaArchivos(String pRutaDescargaArchivos) {
    this.aRutaDescargaArchivos = mc.reemplazarCadena(pRutaDescargaArchivos, ap.obtenerSeparadorArchivos(), String.valueOf('/'));
  }
  ...
  private String interpretarPlantilla(GrupoInformacion pGrupoInformacion, int pValorLlavePrimaria, int pValorLlavePrimariaAnterior, String pContenidoPlantilla) {
    ...
    try {
      ...
      plantilla_local = reemplazarContenidoPlantilla(plantilla_local, valorLlavePrimariaPrincipal_local, pGrupoInformacion.getAplicacion().getIdAplicacion(), valorLlavePrimariaGrupo_local);
    }
  ...
  private String interpretarTablasPlantilla(GrupoInformacion pGrupoInformacion, int pValorLlavePrimaria, int pValorLlavePrimariaAnterior, String pContenidoPlantilla) {
    ...
    try {
      ...
      tablasPlantilla_local = crearTablaPlantilla(tablasPlantilla_local, pGrupoInformacion.getAplicacion().getIdAplicacion(), valorLlavePrimaria_local);
    }
    catch (Exception excepcion) {
      excepcion.printStackTrace();
    } 
  ...
  private String obtenerContenidoPlantillaDocumento(int pIdAplicacionPlantilla, int pPlantillaUtilizar, int pPosicionCampoPlantilla) {
    ...
    try {
      nombreAplicacion_local = getMotorAplicacion().obtenerAplicacionPorId(pIdAplicacionPlantilla).getNombreAplicacion();
      idPrimerGrupo_local = getMotorAplicacion().obtenerIdPrimerGrupoInformacionNoMultipleAplicacion(pIdAplicacionPlantilla);
      
      campoContenido_local = getMotorAplicacion().obtenerCampoPorPosicion(pPosicionCampoPlantilla, idPrimerGrupo_local).getNombreCampo();
      
      if (getAdministradorBaseDatosAplicacion().verificarExistenciaTabla(nombreAplicacion_local)) {
        contenidoPlantillaDocumento_local = getAdministradorBaseDatosAplicacion().obtenerContenidoPlantilla(nombreAplicacion_local, ap.conformarNombreCampoLlavePrimaria(nombreAplicacion_local), pPlantillaUtilizar, campoContenido_local);
  ...
  public void asignarValorPlantillaCamposDocumento(ListaCampo pListaTCampo, int pValorLlavePrimaria, int pValorLlavePrimariaAnterior, int pPlantillaUtilizar) {
    ...
    try {
      ...
      while (iterator_local.hasNext()) {
        ...
        if (campoDocumento_local.getPlantillaCampo().getAplicacionPlantilla() != ConstantesGeneral.VALOR_NULO) {
          contenidoPlantilla_local = obtenerContenidoPlantillaDocumento(campoDocumento_local.getPlantillaCampo().getAplicacionPlantilla().getIdAplicacion(), pPlantillaUtilizar, 2);
          
          contenidoPlantilla_local = interpretarTablasPlantilla(campoDocumento_local.getGrupoInformacion(), pValorLlavePrimaria, pValorLlavePrimariaAnterior, contenidoPlantilla_local);
          
          contenidoPlantilla_local = interpretarPlantilla(campoDocumento_local.getGrupoInformacion(), pValorLlavePrimaria, pValorLlavePrimariaAnterior, contenidoPlantilla_local);
  ...
  public String obtenerContenidoImpresionMasiva(GrupoInformacion pGrupoInformacionActual, int pIdPlantillaUtilizar, String pConsultaPrincipal) {
    ...
    try {
      ...
      if (!mc.esCadenaVacia(pConsultaPrincipal)) {
        resultSet_local = manejadorResultadoConsultaSQL_local.obtenerResultadoConsultaAplicacion(pConsultaPrincipal);
      }
      if (resultSet_local != ConstantesGeneral.VALOR_NULO) {
        generarEncabezado_local = true;
        while (resultSet_local.next()) {
          valorLlavePrimaria_local = resultSet_local.getInt(ap.conformarNombreCampoLlavePrimaria(pGrupoInformacionActual.getAplicacion().getNombreAplicacion()));
          
          if (generarEncabezado_local) {
            contenidoPlantillaEncabezadoDocumento = obtenerContenidoPlantillaDocumento(pGrupoInformacionActual.getAplicacion().getAplicacionImpresionMasiva().getIdAplicacion(), pIdPlantillaUtilizar, 2);
            
            generarEncabezado_local = false;
            encabezadoDocumento_local = interpretarTablasPlantilla(pGrupoInformacionActual, valorLlavePrimaria_local, -1, contenidoPlantillaEncabezadoDocumento);
            
            encabezadoDocumento_local = interpretarPlantilla(pGrupoInformacionActual, valorLlavePrimaria_local, -1, encabezadoDocumento_local);
            
            contenidoImpresionMasiva_local = encabezadoDocumento_local;
          } 
          contenidoPlantillaContenidoDocumento = obtenerContenidoPlantillaDocumento(pGrupoInformacionActual.getAplicacion().getAplicacionImpresionMasiva().getIdAplicacion(), pIdPlantillaUtilizar, 3);
          
          contenidoDocumento_local = interpretarTablasPlantilla(pGrupoInformacionActual, valorLlavePrimaria_local, -1, contenidoPlantillaContenidoDocumento);
          
          contenidoDocumento_local = interpretarPlantilla(pGrupoInformacionActual, valorLlavePrimaria_local, -1, contenidoDocumento_local);
          
          contenidoImpresionMasiva_local = mc.concatenarCadena(contenidoImpresionMasiva_local, contenidoDocumento_local);
        } 
      } 
    } catch (Exception excepcion) {
      excepcion.printStackTrace();
  ...
  public String obtenerValorRegistroCampo(Campo pCampo, int pValorLlavePrimaria, int pValorLlavePrimariaGrupo, String pRegistroConsultar) {
    ...
    try {
      ...
      if (pCampo.getGrupoInformacion().esGrupoInformacionMultiple()) {
        if (mc.esCadenaNumerica(pRegistroConsultar, true)) {
          ...
          if (mc.sonCadenasIguales(pRegistroConsultar, String.valueOf('U'))) {
            posicion_local = getAdministradorBaseDatosAplicacion().obtenerCantidadRegistros(pCampo, pValorLlavePrimaria);
          }
          if (mc.sonCadenasIguales(pRegistroConsultar, String.valueOf('A'))) {
            posicion_local = getAdministradorBaseDatosAplicacion().obtenerPosicionRegistroGrupoInformacion(pCampo, pValorLlavePrimaria, pValorLlavePrimariaGrupo, getAdministradorBaseDatosSisnet().obtenerNombrePrimerCampoValorUnicoGrupoInformacion(pCampo.getGrupoInformacion()));
          }
        } 
      }
      ...
      valorCampo_local = getAdministradorBaseDatosAplicacion().obtenerValorCampoRegistroPorPosicion(pCampo, pValorLlavePrimaria, posicion_local, getAdministradorBaseDatosSisnet().obtenerNombrePrimerCampoValorUnicoGrupoInformacion(pCampo.getGrupoInformacion()));
      ...
      if (esArchivoImagen_local) {
        ...
        if (!mc.esCadenaVacia(valorCampo_local)) {
          ...
          getAdministradorBaseDatosAplicacion().descargarArchivoDeBaseDatos(pCampo, rutaArchivo_local, getAdministradorBaseDatosAplicacion().obtenerValorLlavePrimariaRegistroPorPosicion(pCampo, pValorLlavePrimaria, posicion_local, getAdministradorBaseDatosSisnet().obtenerNombrePrimerCampoValorUnicoGrupoInformacion(pCampo.getGrupoInformacion())));
  ...
  private String obtenerRegistroConsultarPlantilla(String pCampoPlantilla) {
    ...
    try {
      ...
      if (posicionNumeral_local != -1) {
        ...
        registroConsultar_local = mc.obtenerSubCadena(pCampoPlantilla, posicionNumeral_local + 1, posicionFinal_local);
      }
  ...
  private String obtenerContenidoEntreSlash(String pCadena) {
    ...
    try {
      ...
      if (posicionInicial_local != -1 && posicionFinal_local != -1) {
        contenido_local = mc.borrarEspaciosLaterales(mc.obtenerSubCadena(pCadena, posicionInicial_local, posicionFinal_local));
      }
    } catch (Exception excepcion) {
      excepcion.printStackTrace();
    } 
  ...
  private int obtenerPosicionLimiteSubcadenaMas(String pLimiteSubcadena, String pValorCampo) {
    ...
    try {
      subcadenaLimite_local = obtenerContenidoEntreSlash(pLimiteSubcadena);
      caracteresAgregar_local = mc.obtenerSubCadena(pLimiteSubcadena, mc.obtenerPosicionSubCadena(pLimiteSubcadena, String.valueOf('+')) + 1, mc.obtenerLongitudCadena(pLimiteSubcadena));
  ...
  private String asignarSubcadenaValorCampoPlantilla(String pLimitesSubcadena, String pValorCampo) {
    ...
    try {
      ...
      if (posicionInicial_local != -1 && posicionFinal_local != -1 && posicionInicial_local < posicionFinal_local)
      {
  ...
  private String obtenerValorCorrectoPlantilla(String pValorCampoPlantilla, Campo pCampo, int pValorLlavePrimariaPrincipal, int pValorLlavePrimariaGrupo, String pFormatoAplicar, String pEstiloAplicar, String pValorAplicar, String pLimiteSubcadena) {
    ...
    try {
      ...
      if (mc.verificarExistenciaSubCadena(valorCorrectoCampoPlantilla_local, "{$")) {
        valorCorrectoCampoPlantilla_local = reemplazarContenidoPlantilla(valorCorrectoCampoPlantilla_local, pValorLlavePrimariaPrincipal, pCampo.getGrupoInformacion().getAplicacion().getIdAplicacion(), pValorLlavePrimariaGrupo);
      }
      ...
      if (pCampo.esTipoDatoTabla()) {
        ...
        if (mc.esCadenaNumerica(valorCorrectoCampoPlantilla_local, true)) {
          valorCorrectoCampoPlantilla_local = getAdministradorBaseDatosAplicacion().obtenerValorTabla(tabla_local.getNombreTabla(), Integer.parseInt(valorCorrectoCampoPlantilla_local));
        }
      } 
      
      if (pCampo.esCampoEnlazado()) {
        valorCorrectoCampoPlantilla_local = getManejadorCampoEnlazado().obtenerValorCampoEnlazado(pCampo.getEnlaceCampo().getEnlazado(), Integer.parseInt(valorCorrectoCampoPlantilla_local));
      }
      
      valorCorrectoCampoPlantilla_local = ap.obtenerValorCampoConFormato(valorCorrectoCampoPlantilla_local, pCampo.esTipoDatoNumeroEntero(), pCampo.esTipoDatoNumeroReal(), pCampo.esTipoDatoFecha(), pCampo.esTipoDatoHora(), pFormatoAplicar, pValorAplicar);
      
      valorCorrectoCampoPlantilla_local = asignarSubcadenaValorCampoPlantilla(pLimiteSubcadena, valorCorrectoCampoPlantilla_local);
      valorCorrectoCampoPlantilla_local = ap.obtenerValorCampoConEstilo(valorCorrectoCampoPlantilla_local, pEstiloAplicar);
    } catch (Exception excepcion) {
      excepcion.printStackTrace();
    } finally {
      tabla_local = null;
  ...
  private String obtenerValorAplicarPlantilla(String pCampoPlantilla) {
    ...
    try {
      ...
      if (posicionGuion_local != -1 && 
        posicionParentesisCuadrado_local != -1) {
        valorAplicar_local = mc.obtenerSubCadena(pCampoPlantilla, mc.obtenerPosicionSubCadena(pCampoPlantilla, String.valueOf('[')) + 1, mc.obtenerPosicionSubCadena(pCampoPlantilla, String.valueOf(']')));
  ...
  private String obtenerFormatoPlantilla(String pCampoPlantilla) {
    ...
    try {
      ...
      if (posicionGuion_local != -1 && posicionGuion_local != posicionFinalSubcadenaMenos_local) {
        ...
        formatoAplicar_local = mc.obtenerSubCadena(pCampoPlantilla, posicionGuion_local + 1, posicionFinal_local);
      } 
    } catch (Exception excepcion) {
      excepcion.printStackTrace();
    } 
  ...
  private String obtenerValorCampoPlantilla(String pCampoPlantilla, int pValorLlavePrimariaPrincipal, int pIdAplicacion, int pValorLlavePrimariaGrupo) {
    ...
    try {
      ...
      if (mc.comienzaCon(pCampoPlantilla, "{$")) {
        ...
        seudonimo_local = mc.obtenerSubCadena(pCampoPlantilla, 2, obtenerPosicionFinalSeudonimoCampoPlantilla(pCampoPlantilla));
        ...
        if (campo_local != ConstantesGeneral.VALOR_NULO) {
          valorCampoPlantilla_local = obtenerValorRegistroCampo(campo_local, pValorLlavePrimariaPrincipal, pValorLlavePrimariaGrupo, registroConsultar_local);
          
          if (!mc.sonCadenasIguales(valorCampoPlantilla_local, "")) {
            valorCampoPlantilla_local = obtenerValorCorrectoPlantilla(valorCampoPlantilla_local, campo_local, pValorLlavePrimariaPrincipal, pValorLlavePrimariaGrupo, formatoAplicar_local, estiloAplicar_local, valorAplicar_local, limitesSubcadena_local);
          }
        }
  ...
  private String reemplazarContenidoPlantilla(String pContenidoPlantilla, int pValorLlavePrimariaPrincipal, int pIdAplicacion, int pValorLlavePrimariaGrupo) {
    ...
    try {
      ...
      while (mc.verificarExistenciaSubCadena(contenidoPlantilla_local, "{$") && mc.verificarExistenciaSubCadenaAPartirPosicion(contenidoPlantilla_local, "}", mc.obtenerPosicionSubCadena(contenidoPlantilla_local, "{$"))) {
        
        posicionInicial_local = mc.obtenerPosicionSubCadena(contenidoPlantilla_local, "{$");
        
        posicionFinal_local = mc.obtenerPosicionSubCadenaAPartirPosicion(contenidoPlantilla_local, "}", posicionInicial_local) + 1;
        
        if (posicionInicial_local < posicionFinal_local) {
          campoPlantilla_local = mc.obtenerSubCadena(contenidoPlantilla_local, posicionInicial_local, posicionFinal_local);
          ...
          cadenaFinal_local = mc.obtenerSubCadena(contenidoPlantilla_local, posicionFinal_local, mc.obtenerLongitudCadena(contenidoPlantilla_local));
          
          valorCampoPlantilla_local = obtenerValorCampoPlantilla(campoPlantilla_local, pValorLlavePrimariaPrincipal, pIdAplicacion, pValorLlavePrimariaGrupo);
  ...
  private int obtenerCantidadRegistrosCampoPlantilla(String pCampoPlantilla, int pValorLlavePrimaria, int pIdAplicacion) {
    ...
    try {
      if (mc.comienzaCon(pCampoPlantilla, "{$")) {
        ...
        campo_local = getMotorAplicacion().obtenerCampoPorSeudonimo(seudonimo_local, pIdAplicacion);
        if (campo_local != ConstantesGeneral.VALOR_NULO) {
          cantidadRegistros_local = getAdministradorBaseDatosAplicacion().obtenerCantidadRegistros(campo_local, pValorLlavePrimaria);
        }
      }
  ...
  private boolean validarFilaConValores(Campo pPrimerCampoFila, int pValorLlavePrimaria, int pPosicion) {
    ...
    try {
      ...
      valorCampo_local = getAdministradorBaseDatosAplicacion().obtenerValorCampoRegistroPorPosicion(pPrimerCampoFila, pValorLlavePrimaria, pPosicion, getAdministradorBaseDatosSisnet().obtenerNombrePrimerCampoValorUnicoGrupoInformacion(pPrimerCampoFila.getGrupoInformacion()));
  ...
  private String crearTablaPlantilla(String pContenidoPlantilla, int pIdAplicacion, int pValorLlavePrimaria) {
    ...
    try {
      contenidoPlantilla_local = pContenidoPlantilla;
      while (mc.verificarExistenciaSubCadena(contenidoPlantilla_local, "[$IT]") && mc.verificarExistenciaSubCadena(contenidoPlantilla_local, "[$FT]")) {
        ...
        tablaPlantilla_local = mc.obtenerSubCadena(contenidoPlantilla_local, posicionTablaInicial_local + mc.obtenerLongitudCadena("[$IT]"), posicionTablaFinal_local);
        
        cadenaInicial_local = mc.obtenerSubCadena(contenidoPlantilla_local, 0, posicionTablaInicial_local);
        
        cadenaFinal_local = mc.obtenerSubCadena(contenidoPlantilla_local, posicionTablaFinal_local + mc.obtenerLongitudCadena("[$FT]"), mc.obtenerLongitudCadena(contenidoPlantilla_local));
        ...
        if (mc.verificarExistenciaSubCadena(tablaPlantilla_local, "{$")) {
          ...
          while (!encuentracampo_local) {
            posicionFilaInicial_local = mc.obtenerPosicionSubCadenaAPartirPosicion(tablaPlantilla_local, "<tr", posicionFilaFinal_local);
            
            posicionFilaFinal_local = mc.obtenerPosicionSubCadenaAPartirPosicion(tablaPlantilla_local, "</tr>", posicionFilaFinal_local) + mc.obtenerLongitudCadena("</tr>");
            
            tablaInicial_local = mc.obtenerSubCadena(tablaPlantilla_local, 0, posicionFilaInicial_local);
            
            tablaFinal_local = mc.obtenerSubCadena(tablaPlantilla_local, posicionFilaFinal_local, mc.obtenerLongitudCadena(tablaPlantilla_local));
            
            filaPlantilla_local = mc.obtenerSubCadena(tablaPlantilla_local, posicionFilaInicial_local, posicionFilaFinal_local);
          ...
          filaInicial_local = "";
          filaFinal_local = "";
          masCampos_local = (mc.verificarExistenciaSubCadenaAPartirPosicion(filaPlantilla_local, "{$", posicionFinal_local) && mc.verificarExistenciaSubCadenaAPartirPosicion(filaPlantilla_local, "}", posicionFinal_local));
          
          cantidadRegistros_local = -1;
          while (masCampos_local) {
            posicionInicial_local = mc.obtenerPosicionSubCadenaAPartirPosicion(filaPlantilla_local, "{$", posicionFinal_local);
            
            posicionFinal_local = mc.obtenerPosicionSubCadenaAPartirPosicion(filaPlantilla_local, "}", posicionFinal_local) + mc.obtenerLongitudCadena("}");
            
            campoPlantilla_local = mc.obtenerSubCadena(filaPlantilla_local, posicionInicial_local, posicionFinal_local);
            campoPlantilla_local = mc.convertirAMayusculas(campoPlantilla_local);
            filaInicial_local = mc.obtenerSubCadena(filaPlantilla_local, 0, posicionInicial_local);
            
            filaFinal_local = mc.obtenerSubCadena(filaPlantilla_local, posicionFinal_local, mc.obtenerLongitudCadena(filaPlantilla_local));
            
            cantidadRegistros_local = obtenerCantidadRegistrosCampoPlantilla(campoPlantilla_local, pValorLlavePrimaria, pIdAplicacion);
            
            posicionFinalSeudonimo_local = obtenerPosicionFinalSeudonimoCampoPlantilla(campoPlantilla_local);
            seudonimo_local = mc.obtenerSubCadena(campoPlantilla_local, 2, posicionFinalSeudonimo_local);
            
            if (contadorCampos_local == 0) {
              primerCampoFila_local = getMotorAplicacion().obtenerCampoPorSeudonimo(seudonimo_local, pIdAplicacion);
            ...
            if (!mc.verificarExistenciaSubCadena(campoPlantilla_local, String.valueOf('#'))) {
              ...
              seudonimo_local = mc.concatenarCadena("{$", seudonimo_local + "#*" + mc.obtenerSubCadena(campoPlantilla_local, posicionFinalSeudonimo_local, mc.obtenerLongitudCadena(campoPlantilla_local)));
            
            }
            else {
              
              seudonimo_local = mc.concatenarCadena("{$", seudonimo_local + "#*" + mc.obtenerSubCadena(campoPlantilla_local, posicionFinalSeudonimo_local + 2, mc.obtenerLongitudCadena(campoPlantilla_local)));
            } 
          ...
          if (cantidadRegistros_local == 0) {
            ...
            for (contador_local = 1; contador_local <= cantidadRegistros_local; contador_local++) {
              if (validarFilaConValores(primerCampoFila_local, pValorLlavePrimaria, contador_local)) {
                filasPlantilla_local = mc.concatenarCadena(filasPlantilla_local, mc.reemplazarCadena(filaPlantilla_local, "\\#\\*", String.valueOf('#') + contador_local));
              }
            } 
          } 
...
/* Location:              D:\Personal\sisnet\sisnetMasterApp\sisnetapp.war!\WEB-INF\classes\com\sisnet\aplicacion\manejadores\ManejadorPlantilla.class
